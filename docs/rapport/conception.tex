...
Les contraintes de type \emph{NULL}, \emph{NOT NULL} et \emph{DEFAULT} sont ignorées.
	...
	\subsection{Tables}
		Une table (relation) est identifiée par son nom et le schéma auquel elle appartient. Elle possède un ensemble de colonnes (attributs) et un ensemble de contraintes.
	\subsection{Colonne}
		Une colonne (attribut) est identifiée par son nom et la table à laquelle elle appartient. Elle possède un type, et éventuellement une précision. 
